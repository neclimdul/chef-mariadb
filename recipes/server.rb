...
if Chef::Config[:solo]
  ...
  node.default_unless['mariadb']['server_debian_password'] = secure_password
  node.default_unless['mariadb']['server_root_password']   = secure_password
  node.default_unless['mariadb']['server_repl_password']   = secure_password
  node.save
end
...
when 'windows'
  ...
  # Makes absolutely sure this directory exists so the replication script installation below can't fail.
